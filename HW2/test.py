...
def safe_parse(raw_json):
    ...
    try:
        ...
        return None
    if 'created_at' not in json_object:
        return None
    return json_object


#Remember to construct an RDD of (user_id, text) at the end of this cell.
# YOUR CODE HERE
# raise NotImplementedError()
rdd_ut = rdd0.map(safe_parse).filter(lambda x : x != None).map(lambda x : (x['user']['id_str'], x['text']))
...
rdd = rdd_ut.map(lambda x : x[0]).distinct()
c = rdd.count()
...
# raise NotImplementedError()
def helper(ut):
    if(ut[0] in partition):
        return (partition[ut[0]], 1)
    else:
        return (7, 1)
timer_start()
...
# YOUR CODE HERE
# raise NotImplementedError()
rdd = rdd_ut.map(helper).reduceByKey(lambda x,y:x+y).sortByKey()
counts = rdd.collect()
...
# YOUR CODE HERE
# raise NotImplementedError()
#rdd = rdd_ut.flatMapValues(lambda x : list(tok.tokenize(x))).map(lambda x: (x[1], 1)).reduceByKey(lambda x,y: x+y)
rdd_non_dup = rdd_ut.flatMapValues(lambda x : list(tok.tokenize(x))).groupByKey().flatMapValues(lambda x : list(set(x)))
rdd_non_dup.cache() #(user, token) rdd with no duplicates
rdd_token = rdd_non_dup.map(lambda x : (x[1], 1)).reduceByKey(lambda x,y : x+y)
rdd_token.cache() #(token, count) rdd for all tokens
print_count(rdd_token)
...
# YOUR CODE HERE
# raise NotImplementedError()
#rdd = rdd_ut.flatMapValues(lambda x : list(tok.tokenize(x))).groupByKey().mapValues(list)
#print(rdd.collect())
#rdd = rdd.flatMapValues(lambda x : list(set(x)))
#rdd = rdd.map(lambda x : (x[1], 1)).reduceByKey(lambda x,y : x+y)
rdd = rdd_token.filter(lambda x : x[1] >= 100).map(lambda x : (x[1], x[0]))
rdd = rdd.sortByKey(ascending=False).map(lambda x : (x[1], x[0]))
print_count(rdd)
tokens = rdd.take(20)
print_tokens(tokens)
...
#rdd = rdd_ut.flatMapValues(lambda x : list(tok.tokenize(x))).groupByKey().mapValues(list)
#rdd = rdd.flatMapValues(lambda x : list(set(x)))
# convert to ((token,group),1),reduce to count for each token in each group
rdd = rdd_non_dup.map(lambda x: ((x[1], helper(x[0])), 1)).reduceByKey(lambda x,y: x+y)
# convert to (token,(group,count))
rdd = rdd.map(lambda x: (x[0][0], (x[0][1], x[1])))
# group by token, get sum of all group, convert to (token,(all,list))
rdd = rdd.groupByKey().mapValues(lambda x : (sumOfList(list(x)), list(x))).filter(lambda x : x[1][0] >= 100)
# flat by each group
rdd = rdd.map(lambda x : ((x[0], x[1][0]), x[1][1])).flatMapValues(lambda x:x)
rdd = rdd.map(lambda x : ((get_rel_popularity(x[1][1], x[0][1]), x[0][0]), x[1][0]))

for i in range(8):
    rdd_tmp = rdd.filter(lambda x : x[1] == i).sortBy(lambda x : (-x[0][0], x[0][1])).map(lambda x : (x[0][1], x[0][0]))
    print_tokens(rdd_tmp.take(10), i)
